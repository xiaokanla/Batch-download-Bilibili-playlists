# BiliDownloader Studio

BiliDownloader Studio 是一个面向 B 站收藏夹批量下载、历史去重、Eagle 视频导入和视频套图封面生成的本地 Web 工具。

> 当前源码版本：`v1.4.6`

## 下载

普通用户建议直接下载 Release 里的正式版压缩包，解压后运行 `BiliDownloaderStudio.exe`，不需要配置 Python 环境。

最新正式包请以 GitHub Release 页面为准。正式包统一命名为
`BiliDownloaderStudio-vX.Y.Z-windows-x64.zip`，校验文件为同名的
`.sha256` 文件。

## 主要功能

- 扫码登录 B 站并同步收藏夹；支持不先退出当前账号的扫码切换账号。
- 按账号名称、UID 或空间主页查找 UP 主，获取公开投稿后用顶部搜索栏筛选并批量下载。
- 按收藏夹、月份、时长、下载状态筛选视频。
- 按近 3 个月、近 6 个月、近 1 年或指定月份生成标签关系蛛网图；节点代表标签，连线代表标签在同一视频中共现，点击节点即可筛选对应视频。
- 提供独立的 Eagle“标签蛛网图”插件，在 Eagle 内分析全库、当前文件夹或选中项目的标签关系。
- 在视频列表右键即可打开对应的 B 站视频网页或复制 BV 号。
- 批量下载选中视频，支持 2K/4K 优先、暂停、取消、限速、音频下载和分 P 下载。
- 导入/导出历史下载记录，换设备后避免重复下载。
- 一键打开下载记录文件位置，方便迁移记录。
- 将已下载视频导入 Eagle 指定文件夹。
- 为 Eagle 视频生成套图缩略图：优先使用 B 站封面和弹幕峰值帧，匹配不到 BV 时自动退回本地抽帧。
- 自定义路径：数据目录、下载目录、FFmpeg、FFprobe、Aria2、Eagle 缓存导出目录、错误日志。
- 内置缓存管理：查看数据库、收藏夹、标签、Eagle 缓存和下载目录的实际位置与占用，并可校验后迁移程序数据或 Eagle 生成缓存。
- 网页 Eagle 导入和封面生成任务默认不弹额外系统窗口，进度和错误都显示在网页里。

Eagle 插件源码位于 `eagle_plugin_tag_graph/`，不依赖下载器后端、Cookie 或个人数据。
安装和开发说明见该目录下的 `README.md`。

## 快速开始

### 使用 exe

1. 从 GitHub Release 下载正式版压缩包。
2. 解压到任意目录。
3. 双击 `BiliDownloaderStudio.exe`。
4. 程序会自动打开本地网页界面。
5. 按界面里的新手引导完成登录、同步、下载或 Eagle 导入。

程序首次启动会在数据目录中创建 `bili_downloader.db`。旧版本的 JSON
记录会自动迁移，原 JSON 文件不会删除；迁移完成后程序以 SQLite 为主存储，
同时继续更新 JSON 镜像，方便旧版工具和独立 Eagle 脚本继续使用。

### 从源码运行

1. 安装 Python 3.10 或更高版本。
2. 安装依赖：

```bash
pip install -r requirements.txt
```

3. 启动：

```bash
python web_app.py
```

4. 浏览器打开：

```text
http://127.0.0.1:8765
```

## Eagle 导入说明

1. 先打开 Eagle。
2. 在 BiliDownloader 右侧 Eagle 区域选择 `.library` 目录。
3. 选择目标 Eagle 文件夹。
4. 选择速度模式。
5. 点击“导入已下载视频到 Eagle”。
6. 任务进度、成功数、失败数和错误信息会显示在网页面板里。

如果勾选“导入成功后删除本地视频”，程序会在 Eagle 导入和封面处理成功后删除下载目录里的源视频，以节省空间。

## 文档

- [更新日志](CHANGELOG.md)
- [用户指南](docs/USER_GUIDE.md)
- [开发者指南](docs/DEVELOPER_GUIDE.md)
- [发布指南](docs/RELEASE_GUIDE.md)

## 风控原则

本项目默认采用保守策略：缓存数据、低频请求、优先使用本地记录，不通过高并发请求提高速度。收藏夹同步、账号投稿分页和标签读取均使用低频串行请求；标签只会在手动点击“生成关系图”后读取，并会写入本地缓存。检测到 `-412`、`-352`、`403`、`412` 或 `429` 等风控响应会立即停止。请不要把并发、请求频率或重试策略改得过于激进。

## 开源提醒

不要提交以下个人文件：

- `last_login_cookie.json`
- `bili_netscape_temp.txt`
- `userdata/`
- Eagle 库目录
- 下载视频目录
- 任何 Cookie、账号、私人收藏夹缓存

## 最新修复

### v1.4.6

- 新增缓存管理面板，可查看程序数据、数据库、标签、Eagle 缓存和日志的实际位置与占用。
- 支持校验迁移程序数据或 Eagle 生成缓存，默认保留旧目录，也可完整校验后清理旧缓存。
- 新增无损切换 B站账号，以及 2K/4K 优先批量下载模式。
- 优化直链下载：在总连接数受控的前提下并行下载音视频流，并自动切换备用 CDN 或内置下载器。
- 未登录时展示独立登录舞台，登录后再展开完整工作区。

### v1.4.5

- 主工作区支持拖动调节高度、收起和自适应，为视频列表释放更多空间。
- 新增浅色主题，并统一优化收藏夹、筛选条件和 Eagle 等下拉菜单。
- 页码支持直接输入跳转，视频右键菜单新增单视频下载、状态切换和复制信息。
- 右侧环境诊断、标签分析、Eagle 和日志等面板可独立收起，并记住使用状态。

### v1.4.4

- 重构账号投稿界面，账号搜索、候选账号选择和投稿获取形成独立工作流。
- 候选账号改为头像卡片，直接显示名称、粉丝数和 UID。
- 新增轻量界面动效、即时列表选择以及 Ctrl+A、Ctrl+F、方向键、Space、Enter、Esc 等快捷操作。
- 修复账号搜索遇到空响应、非 JSON 风控页时显示底层解析错误的问题，并支持旧缓存降级。

### v1.3.0-tag-cloud

- 移除账号投稿区的独立关键词输入，统一使用顶部搜索栏筛选已获取的投稿列表。
- 新增标签关系蛛网图：支持近 3/6/12 个月、指定月份和“仅已下载”分析，节点表示标签，连线表示标签共现关系。
- 点击关系图节点可直接筛选当前视频列表；标签读取手动触发、低频串行并持久化缓存。

### v1.4.0-sqlite

- 新增 SQLite 主数据存储，统一保存下载记录、历史、收藏夹缓存、搜索缓存、标签缓存和 Eagle 配置。
- 首次运行自动迁移旧 JSON，保留 JSON 镜像和原有导入/导出功能，换设备时可直接迁移数据目录或历史记录包。
- 使用 WAL、事务和索引降低大规模记录读取时的等待，减少每次操作反复解析完整 JSON 的开销。

### v1.4.3-eagle-tag-repair

- 下载完成后低频缓存视频 B 站标签，写入数据库和单视频标签文件。
- Eagle 导入时可把已缓存的 B 站标签追加到视频，并自动归入独立的 `BiliDownloader 标签` 标签组。
- 对已经导入 Eagle、但本地视频已删除的旧项目，可仅根据 BV 号同步标签，不重复导入视频。
- 标签同步会通过 Eagle 更新接口刷新项目和标签组；同步结束后会清理插件标签组中的未使用标签。

### v1.2.9-creator-source

- 新增独立的“账号投稿”工作区，不与收藏夹列表混在一起。
- 支持按账号名称、UID 或空间主页选择 UP 主，再获取其公开投稿。
- 账号检索和投稿分页采用低频串行请求，遇到风控响应立即停止。

### v1.2.7-dropdownfix

- 修复收藏夹下拉框被“筛选”面板遮挡的问题。
- 静态 UI 文件改为禁缓存加载，避免运行旧页面导致修复不生效。
- 新增“打开记录位置”按钮，方便迁移 `download_records.json`。

### v1.2.6-nopopup

- 修复网页导入 Eagle 时可能弹出大量系统窗口的问题。
- 修复 Eagle 导入流程误触发 `tkinter.ttk` 导致失败的问题。
- 修复打包版缺少 `PIL.ImageStat` 导致封面/套图生成失败的问题。
- 让 `ffmpeg` / `ffprobe` 后台运行时不再弹出额外窗口。
- 正式版隐藏“恢复初始状态”，测试版保留该功能。
